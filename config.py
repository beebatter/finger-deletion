...
CASCADE = {
    ...
    # 最终相似度融合权重
    "deep_weight": 0.86,     # 深度特征在同源比对时极高(>95%)，设为主导权重
    "minutiae_weight": 0.14, # 细节点区分度极高(异源0%)但同源分布偏低(60%-70%)，作为辅助验证项
    
    # 【重构：常规识别设定】1:1判定最终得分阈值（设为88%，超过此分则判定为同一枚指纹）
    "match_decision_threshold": 0.88, 
}
